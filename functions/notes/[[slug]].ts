...
function rewriteMarkdownAssets(markdown: string, slug: string) {
  ...
  return markdown.replace(/!\[([^\]]*)\]\(([^)]+)\)/g, (match, alt, src) => {
    ...
    return `![${alt}](${rawBase}/${notesOwner}/${notesRepo}/main/${encodePath(normalized)})`;
  });
}
...
function pageHtml({
  ...
  const sourceUrl = `https://github.com/${notesOwner}/${notesRepo}/blob/main/${encodePath(slug)}.md`;
...
async function fetchRawMarkdown(slug: string, env: Env) {
  ...
  try {
    const response = await fetch(
      `${rawBase}/${notesOwner}/${notesRepo}/main/${encodePath(slug)}.md`,
      { headers: requestHeaders },
    );

    if (response.ok) return response.text();
  } catch (error) {
    // Fall through to the GitHub Contents API when the raw host is unavailable.
  }

  const response = await fetch(
    `${apiBase}/repos/${notesOwner}/${notesRepo}/contents/${encodePath(slug)}.md?ref=main`,
    {
      headers: {
        ...requestHeaders,
